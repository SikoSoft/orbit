...
Chart data and dataset labels are managed through `src/lib/ChartUtil.ts`. All components that render charts must derive their dataset label using `getChartDatasetLabel(dataPoints)` from that module — never derive labels inline. This function handles all `FactOperation` types including `analysisClassification`, which maps to per-type keys (`chart.analysisClassificationType.*` in the localization file) instead of the generic `factOperation.*` label.

When adding a new chart rendering site:
1. Import `convertResponseToChartData` and `getChartDatasetLabel` from `@/lib/ChartUtil`.
2. Call `getChartDatasetLabel(chart.config.dataPoints)` to get the label, then pass it to `convertResponseToChartData`.

When adding a new `AnalysisClassificationType` value:
1. Add the enum value in `api-spec`.
2. Add a `chart.analysisClassificationType.{value}` key to `src/lib/data/localization/en.json`.
3. No changes to `ChartUtil.ts` are required — it uses the enum value as the key suffix directly.
...
- Avoid optional chaining (`?.`) to guard against nulls. Instead, use early returns or conditional rendering (`return nothing`) to handle the null case explicitly before the value is needed. This keeps the happy path clean and makes null handling visible and intentional.
