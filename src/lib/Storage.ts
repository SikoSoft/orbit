...
export class Storage implements StorageSchema {
  ...
  @delegateSource()
  async getEntity(_id: number): Promise<Entity.Entity | null> {
    return Promise.resolve(null);
  }
  ...
  @delegateSource()
  async deleteChart(_id: number): Promise<boolean> {
    return Promise.resolve(false);
  }

  @delegateSource()
  async getStreaks(): Promise<{ streaks: Streak[]; results: StreakResult[] }> {
    return Promise.resolve({ streaks: [], results: [] });
  }

  @delegateSource()
  async createStreak(
    _name: string,
    _context: StreakContext,
  ...
  async updateStreak(
    ...
    _context: StreakContext | undefined,
  ): Promise<StorageResult<Streak>> {
    return Promise.resolve({ isOk: false, error: new Error('Not implemented') });
  }
  ...
  getActiveWorkspaceTheme(): ThemeName {
    return (localStorage.getItem(StorageItemKey.ACTIVE_WORKSPACE_THEME) as ThemeName) ?? defaultTheme;
  }
